...
class FavAnimeViewModel @Inject constructor(
    ...
    internal fun getRandomAnimeList(): LiveData<NetRes<AnimeList?>> = randomAnimeList
            ...
            .subscribeWith(object : DisposableSingleObserver<Response<Anime>>() {
                override fun onSuccess(response: Response<Anime>) {
                    ...
                    if (response.code() == HttpURLConnection.HTTP_OK) {
                        ...
                        anime.postValue(ApiState.Error(message = errorMessage))
                    }
...
//            filteredAnimeList.postValue(NetRes(status = Status.LOADING, loadingState = LoadingState.SHOW))
            ...
            if (response.code() == HttpURLConnection.HTTP_OK) {
                ...
                val errorMessage = utils.getErrorMessageWithRetrofit(context = getApplication(), errorResponseBody = response.errorBody())
...
//            filteredAnimeList.postValue(NetRes(status = Status.LOADING, loadingState = LoadingState.HIDE))
    ...
    internal fun loadRandomAnimeList() = viewModelScope.launch {
        randomAnimeList.postValue(repository.getRandomAnimeList().value)
    }
}
